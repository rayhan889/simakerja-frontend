...
import { activityLabels, documentTypeLabels, partnerCooperationPeriodOptions, studyProgramOptions, type MoaIASubmissionMode, type PartnerAndFacultyProfile, type StudyProgram } from '@/types/submission.type';
...
const moaIaDetailSchema = z.object({
    ...
    facultyRepresentativeName: z.literal(FACULTY_REPRESENTATIVE_NAME),
    ...
    mode: z.enum(['new_partner', 'existing_partner'], { error: "Mode mitra harus dipilih" }),
});
...
const buildDefaultValues = (userStudyProgram?: string): SubmissionFormValues => {
    return {
        ...
        moaIa: {
            ...
            mode: 'existing_partner',
            studentSnapshots: [
                {
                    studyProgram: userStudyProgram ?? '',
                    students: [],
                    unit: '',
                },
            ],
...
export const DashboardSubmitSubmissionPage = () => {
    ...
    const form = useForm<SubmissionFormValues>({
        ...
        defaultValues: buildDefaultValues(userStudyProgram),
        mode: 'onChange'
    })
    ...
    const [partnerMode, setPartnerMode] = useState<MoaIASubmissionMode>('existing_partner');

    const [selectedPartner, setSelectedPartner] = useState<PartnerAndFacultyProfile | null>(null);

    const isPartnerFieldDisabled = partnerMode === 'existing_partner' || selectedPartner !== null;
    ...
    useEffect(() => {
        ...
        if (partnerMode !== 'new_partner' || !formPartnerName || !formPartnerNumber) {
            return;
        }
    ...
    const handlePartnerModeChange = useCallback((mode: MoaIASubmissionMode) => {
        setPartnerMode(mode);
        form.setValue('moaIa.mode', mode, { shouldDirty: true });
    ...
    const handleReset = () => {
        form.reset(buildDefaultValues(userStudyProgram));
        setPartnerLogoPreviewUrl(null);

        setPartnerMode('existing_partner');
        setSelectedPartner(null);
    }
                            ...
                            <div className="bg-white rounded-lg border border-gray-200 w-full flex flex-col items-start p-5 gap-y-6 ">
                                ...
                                <div className="flex flex-col gap-2 w-full">
                                    <SegmentedControl
                                        ...
                                        options={[
                                            { value: 'existing_partner', label: 'Pilih Mitra Yang Ada' },
                                            { value: 'new_partner', label: 'Buat Mitra Baru' },
                                        ]}
                                    />
                                    <p className="text-xs text-gray-500 ">
                                        {partnerMode === 'existing_partner'
                                ...
                                <div className='w-full flex flex-col gap-3'>
                                    ...
                                    <div className='grid grid-cols-1 md:grid-cols-4 gap-3 w-full'>
                                            ...
                                            render={({ field }) => (
                                                <FormItem className='text-start flex flex-col space-y-2'>
                                                    ...
                                                    <FormControl>
                                                        <Input
                                                            {...field}
                                                            placeholder={FACULTY_REPRESENTATIVE_NAME}
                                                            disabled
                                                        />
                                                    </FormControl>
                                                    <FormMessage />
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem className='text-start flex flex-col space-y-2'>
                                                    <FormLabel>Nomor Mitra</FormLabel>
                                                    <FormControl>
                                                        <Input
                                                            {...field}
                                        ...
                                        {partnerMode === 'existing_partner' && selectedPartner && (
